...
<body>
	...
	<div>
		...
		<form action="<%=request.getContextPath()%>/parking-lot-edit" method="post"
			class="float-left main-content border-top" id="form-update-employee">
			<h2 class="m-4">Edit Parking lot</h2>
			<hr class="m-4">
			<input type="hidden" name="id" value="${parkingLot.id}" id="id">
			<table class="m-4">
				<tr>
					...
					<td><input class="box form-control pt-2 pb-2 text-sm"
						...
						id="parkingname" required value="${parkingLot.name}"></td>
				</tr>
				<tr>
						...
						class="box form-control pt-2 pb-2 text-sm">
							<c:forEach items="${parkingPlaces}" var="place">
								<option value="${place.id}"
									${place.id==parkingLot.id?"selected":""}>${place.name}</option>
							</c:forEach>
					</select></td>
				</tr>
				<tr>
					...
					<td><input class="box form-control pt-2 pb-2 text-sm"
						...
						min="1" required value="${parkingLot.area}"></td>
				...
				<tr>
					...
					<td><input class="box form-control pt-2 pb-2 text-sm"
						...
						min="1" required value="${parkingLot.price}"></td>
